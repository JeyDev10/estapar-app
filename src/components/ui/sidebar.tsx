...
import { ComponentProps, createContext, useContext, CSSProperties, useState, useCallback, useMemo } from "react"
    ...
    >
      ...
      >
        <div
          ...
          className="bg-bg-primary group-data-[variant=floating]:border-gray-tertiary flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
...
function SidebarHeader({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("flex flex-col gap-2 p-4 border border-gray-tertiary", className)}
      {...props}
    />
  )
}
...
function SidebarSeparator({ className, ...props }: ComponentProps<typeof Separator>) {
  return (
    <Separator
      ...
      className={cn("bg-gray-tertiary mx-2 w-auto", className)}
      {...props}
    />
  )
}
...
function SidebarMenuButton({
  ...
  const button = (
    <Comp
      ...
      className={cn(sidebarMenuButtonVariants({ variant, size }), "cursor-pointer p-6 rounded-lg", className)}
      {...props}
    />
  )
